...
def save_checkpoint(checkpoint_path, rlhf_engine, trainer, epoch, step, args):
    ...
    # rlhf_engine.ref = convert_lora_to_linear_layer(rlhf_engine.ref)
    # rlhf_engine.reward = convert_lora_to_linear_layer(rlhf_engine.reward)
    # rlhf_engine.tokenizer = convert_lora_to_linear_layer(rlhf_engine.tokenizer)
    ...
    torch.save({
        'epoch': epoch,
        'step': step,
        'rlhf_engine_actor_state_dict': rlhf_engine.actor.state_dict(),
...
def main():
    ...
    if args.resume_checkpoint:
        ...
        rlhf_engine.actor.load_state_dict(checkpoint['rlhf_engine_actor_state_dict'])
        rlhf_engine.critic.load_state_dict(checkpoint['rlhf_engine_critic_state_dict'])
        rlhf_engine.ref.load_state_dict(checkpoint['rlhf_engine_ref_state_dict'])
        rlhf_engine.reward.load_state_dict(checkpoint['rlhf_engine_reward_state_dict'])
        # rlhf_engine.tokenizer.load_state_dict(checkpoint['rlhf_engine_tokenizer_state_dict'])
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        print_rank_0(
            ...
            args.global_rank)
        # for step in range(start_step, len(prompt_train_dataloader)):
        #     print(len(prompt_train_dataloader))
        #     batch_prompt = prompt_train_dataloader[step]
        #     batch_unsupervised = unsupervised_train_dataloader[step]
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if (step + 1) % args.save_interval == 0:
                checkpoint_path = os.path.join(args.output_dir, f'checkpoint_epoch_{epoch}_step_{step + 1}.pt')
                save_checkpoint(args.output_dir, rlhf_engine, trainer, epoch, step, args)
